...
import io.tinyrpc.reflect.asm.visitor.TargetClassVisitor;
...
//            save2File(outputFile, bytes);
	...
	private static void addConstructor(ClassWriter writer,
									   List<TargetClassVisitor.MethodBean> constructors,
	 ...
	 * 添加继承的方法或目标类本身的方法
	 */
	private static int addMethod(ClassWriter writer,
								 String newClassInnerName,
								 Method[] methods,
								 List<TargetClassVisitor.MethodBean> methodBeans,
								 boolean isPublic,
								 int methodNameIndex,
								 Map<Integer, Integer> map) throws Exception {
		for (TargetClassVisitor.MethodBean methodBean : methodBeans) {
	...
	private static void addMethod(ClassWriter writer,
								  String newClassInnerName,
								  TargetClassVisitor.MethodBean methodBean,
								  int access, int methodNameIndex) throws Exception {
	 ...
	 * 添加静态初始代码块
	 */
	private static void addStaticInitBlock(ClassWriter writer,
										   String targetClassName,
										   String newClassInnerName,
										   Map<Integer, Integer> methodsMap,
	...
	private static Class<?> transfer2Class(ClassLoader classLoader, byte[] bytes) {
		try {
			...
			Method defineClassMethod = cl.getDeclaredMethod(
				"defineClass",
